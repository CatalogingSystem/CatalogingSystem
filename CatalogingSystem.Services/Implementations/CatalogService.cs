...
public class CatalogService : ICatalogService
{
            ...
            .Select(x => new CatalogItemDto
            {
                ...
                Identification =
                    x.Identification != null
                        ? _mapper.Map<IdentificationDto>(x.Identification)
                        : null,
                DescriptionClassification =
                    x.DescriptionClassification != null
                        ? _mapper.Map<DescriptionClassificationDto>(x.DescriptionClassification)
                        : null,
                AdministrativeData =
                    x.AdministrativeData != null
                        ? _mapper.Map<AdministrativeDataDto>(x.AdministrativeData)
                        : null,
                Conservation =
                    x.Conservation != null ? _mapper.Map<ConservationDto>(x.Conservation) : null,
                GraphicDocumentation =
                    x.GraphicDocumentation != null
                        ? _mapper.Map<GraphicDocumentationDto>(x.GraphicDocumentation)
                        : null,
    ...
    public async Task<CatalogItemDto?> GetCatalogItem(long expediente)
    {
        ...
        if (result == null || result.Archivo == null)
            return null;
        ...
        return new CatalogItemDto
        {
            ...
            Identification =
                result.Identification != null
                    ? _mapper.Map<IdentificationDto>(result.Identification)
                    : null,
            DescriptionClassification =
                result.DescriptionClassification != null
                    ? _mapper.Map<DescriptionClassificationDto>(result.DescriptionClassification)
                    : null,
            AdministrativeData =
                result.AdministrativeData != null
                    ? _mapper.Map<AdministrativeDataDto>(result.AdministrativeData)
                    : null,
            Conservation =
                result.Conservation != null
                    ? _mapper.Map<ConservationDto>(result.Conservation)
                    : null,
            GraphicDocumentation =
                result.GraphicDocumentation != null
                    ? _mapper.Map<GraphicDocumentationDto>(result.GraphicDocumentation)
                    : null,
    ...
    )
    {
        ...
        if (
            ...
            || !string.IsNullOrEmpty(genericClassification)
        )
        {
            query = query.Where(x =>
                x.Identification != null
                && (
                    string.IsNullOrEmpty(materialName)
                    || EF.Functions.ILike(
                        x.Identification.material.MaterialName,
                        $"%{materialName}%"
                    )
                )
                && (
                    string.IsNullOrEmpty(authorName)
                    || EF.Functions.ILike(x.Identification.author.Name, $"%{authorName}%")
                )
                && (
                    string.IsNullOrEmpty(titleName)
                    || EF.Functions.ILike(x.Identification.title.Name, $"%{titleName}%")
                )
                && (
                    string.IsNullOrEmpty(genericClassification)
                    || EF.Functions.ILike(
                        x.Identification.genericClassification,
    ...
    public async Task<bool> DeleteCatalogItem(long expediente)
    {
        ...
        if (archivo.peticionTransferencia == true)
        {
            throw new InvalidOperationException(
                "No se puede eliminar el archivo porque hay una petición de transferencia pendiente."
            );
        }
        var temporalMovements = await _context
            .TemporalMovements.Where(tm => tm.Expediente == expediente)
            .ToListAsync();
        ...
        var conservation = await _context.Conservations.FirstOrDefaultAsync(c =>
        ...
        return new CatalogItemDto
        {
            ...
            GraphicDocumentation =
                result.GraphicDocumentation != null
                    ...
                    : null,
            Dating = result.Dating != null ? _mapper.Map<DatingDto>(result.Dating) : null,
            TemporalMovements = result
                .TemporalMovements.Select(tm => _mapper.Map<TemporalMovementDto>(tm))
                .ToList(),
        };
    }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Expedientes duplicados encontrados en el archivo: {string.Join(", ", duplicateExpedientes)}"
                );
            }

            foreach (var item in catalogItems)
            {
                long efectivoExpediente = nuevoExpediente.HasValue
                ...
                {
                    var existingArchivo =
                        await _context.ArchivosAdministrativos.FirstOrDefaultAsync(a =>
                            a.expediente == item.Expediente
                        );
                    if (existingArchivo != null)
                    {
                        throw new InvalidOperationException(
                            $"El expediente {item.Expediente} ya existe."
                        );
                    }
                }

                item.Expediente = efectivoExpediente;
                if (item.ArchivoAdministrativo != null)
                    item.ArchivoAdministrativo.Expediente = efectivoExpediente;
                if (item.Identification != null)
                    item.Identification.Expediente = efectivoExpediente;
                if (item.DescriptionClassification != null)
                    item.DescriptionClassification.Expediente = efectivoExpediente;
                if (item.AdministrativeData != null)
                    item.AdministrativeData.FileNumber = efectivoExpediente;
                if (item.Conservation != null)
                    item.Conservation.Expediente = efectivoExpediente;
                if (item.GraphicDocumentation != null)
                    item.GraphicDocumentation.Expediente = efectivoExpediente;
                if (item.Dating != null)
                    item.Dating.Expediente = efectivoExpediente;

                var archivo = await _archivoService.CreateArchivoAdministrativo(
                    item.ArchivoAdministrativo
                );
                ...
                if (item.DescriptionClassification != null)
                {
                    await _descriptionService.CreateDescriptionClassification(
                        item.DescriptionClassification
                    );
                }
                ...
                if (item.TemporalMovements != null && item.TemporalMovements.Any())
                {
                    foreach (var movementDto in item.TemporalMovements)
                    {
                        ...
                        await _context.TemporalMovements.AddAsync(
                            _mapper.Map<TemporalMovement>(movementDto)
                        );
                    }
                }
            }
