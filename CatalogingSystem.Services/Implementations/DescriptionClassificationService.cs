...
public class DescriptionClassificationService : IDescriptionClassificationService
{
    ...
    public DescriptionClassificationService(ApplicationDbContext context, IMapper mapper, IAuditService auditService)
    {
        _context = context;
        _mapper = mapper;
    ...
    {
        var descriptions = await _context.DescriptionClassifications
    ...
    public async Task<DescriptionClassification> CreateDescriptionClassification(DescriptionClassificationDto dto)
    {
        var archivo = await _context.ArchivosAdministrativos
            .FirstOrDefaultAsync(a => a.expediente == dto.Expediente);
        if (archivo == null)
        {
            throw new InvalidOperationException($"No administrative file exists with expediente {dto.Expediente}");
        }

        bool exists = await _context.DescriptionClassifications
            .AnyAsync(d => d.Expediente == dto.Expediente);
        if (exists)
        {
            throw new InvalidOperationException($"Description classification already exists for expediente {dto.Expediente}");
        }
    ...
    public async Task<bool> UpdateDescriptionClassification(long expediente, UpdateDescriptionClassificationDto dto)
    {
        var description = await _context.DescriptionClassifications
            .FirstOrDefaultAsync(d => d.Expediente == expediente);
